...
if __name__ == "__main__":
    ...
    try:
        dbconn = proteus.createConnection().cursor()
                    ...
                    if (line.lower().startswith("select ") or is_test):
                        ...
                        for rep in range(5+1):
                            ...
                            if create_test is None or gentests_exec:
                                ...
                                hwconf = {}
                                if hybrid:
                                    hwconf["hwmode"] = "hybrid"
                                elif cpu_only:
                                    hwconf["hwmode"] = "cpu"
                                else:
                                    hwconf["hwmode"] = "gpu"
                                hwconf["cpudop"] = num_of_cpus
                                hwconf["gpudop"] = num_of_gpus
                                print(hwconf)
                                dbconn.alterSession(hwconf)
                                print(sql_query)
                                res = dbconn.executeTimedQuery(sql_query)
                                print(res[0])
                                if (timings):
                                    t2 = time.time()
                                    total_ms = (t2 - t0) * 1000
                                    ...
                                    if (not is_warmup) and rep > 0: # rep == 0 is used for warming up the storage manager
                                        total_ms        = res[1]["total_time"]
                                        plan_ms         = res[1]["planning_time"]
                                        wplan_ms        = res[1]["plan2json_time"]
                                        wexec_ms        = res[1]["executor_time"]
                                        Tcodegen        = res[1]["codegen_time"]
                                        Tdataload       = res[1]["dataload_time"]
                                        Tcodeopt        = res[1]["code_opt_time"]
                                        Tcodeoptnload   = res[1]["code_optnload_time"]
                                        Texec           = res[1]["execution_time"]
                                        print(total_ms)
                                        print([total_ms, plan_ms, wplan_ms, wexec_ms, Tcodegen, Tdataload, Tcodeopt, Tcodeoptnload, Texec])
                                        sheets_append([total_ms, plan_ms, wplan_ms, wexec_ms, Tcodegen, Tdataload, Tcodeopt, Tcodeoptnload, Texec], label, sql_query, conf)
                                    if rep > 0:
                                        if timings_csv:
                                            print('Timing,' +
